...
class SellerRegistrationService {
  ...
  static Future<SellerRegistrationResponse> sellerRegister(SellerRegistrationData sellerRegistrationData) async {
    ...
    try {
      ...
      */
      ...
      request.fields.addAll({
        ...
        'commercial_document': sellerRegistrationData.commercialDocument.toString(),
        'cr_id_no': sellerRegistrationData.crIDNo,
      });

      request.files.add(await http.MultipartFile.fromPath('cr_freelance_document', sellerRegistrationData.crFreelanceDocPath));
      request.files.add(await http.MultipartFile.fromPath('image', sellerRegistrationData.imagePath));
      request.files.add(await http.MultipartFile.fromPath('logo', sellerRegistrationData.logoPath));
      request.files.add(await http.MultipartFile.fromPath('banner', sellerRegistrationData.bannerPath));
      request.files.add(await http.MultipartFile.fromPath('additional_legal_document', sellerRegistrationData.additionalLegalDocPath));
      ...
      if (response.statusCode == 200) {
        ...
        return SellerRegistrationResponse(success: false, message: 'Unauthorized');
      }
    ...
    on SocketException {
      return SellerRegistrationResponse(success: false, message: 'Not connect to internet !');
    }
    on TimeoutException catch (e) {
      return SellerRegistrationResponse(success: false, message: 'Request timeout');
    }
    on FormatException catch (e) {
      return SellerRegistrationResponse(success: false, message: 'Bad response format');
    }
    finally {
      EasyLoading.dismiss();
    }

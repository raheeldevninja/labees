  ...
  static Future<AllFAQs> getFAQs(String lang) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        List<FAQResponse> faqs = (result as List)
            .map((data) => FAQResponse.fromJson(data))
            .toList();
  ...
  static Future<FooterSettingsResponse> getFooterSettings() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return FooterSettingsResponse(success: false, message: result['errors'][0]['message']);
      }
      else if (response.statusCode == 500) {
        return FooterSettingsResponse(success: false, message: 'Server Error');
      }
      else {
        return FooterSettingsResponse(success: false, message: 'Something went wrong !');
      }
    }
    on SocketException {
      return FooterSettingsResponse(success: false, message: 'Not connect to internet !');
    }
    on TimeoutException catch (e) {
      return FooterSettingsResponse(success: false, message: 'Request timeout');
    }
    on FormatException catch (e) {
      return FooterSettingsResponse(success: false, message: 'Bad response format');
    }
    finally {
      EasyLoading.dismiss();
    }
  ...
  static Future<PageDetailsResponse> getPageDetails(String slug) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return PageDetailsResponse(success: false, message: result['errors'][0]['message']);
      }
      else if (response.statusCode == 500) {
        return PageDetailsResponse(success: false, message: 'Server Error');
      }
      else {
        return PageDetailsResponse(success: false, message: 'Something went wrong !');
      }
    }
    on SocketException {
      return PageDetailsResponse(success: false, message: 'Not connect to internet !');
    }
    on TimeoutException catch (e) {
      return PageDetailsResponse(success: false, message: 'Request timeout');
    }
    on FormatException catch (e) {
      return PageDetailsResponse(success: false, message: 'Bad response format');
    }
    finally {
      EasyLoading.dismiss();
    }

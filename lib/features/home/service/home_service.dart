...
class HomeService {
  ...
  static Future<DashboardData> getDashboardData(
    ...
    String url =
        '${APIs.baseURL}${APIs.dashboardData}?category_id=$categoryId&allowed_data=$allowedData';
  ...
  static Future<ProductModel> getProducts(
    ...
    url =
        '${APIs.baseURL}${APIs.productListing}$id?limit=$limit&offset=$offset&sort=$sort';

    if (minAmount != null) {
      url +=
          '&filters[min_amount_filter]=$minAmount&filters[max_amount_filter]=$maxAmount';
    }
  ...
  static Future<AddWishListResponse> addToWishlist(
      String lang, int productId) async {
  ...
  static Future<WishListRemoveResponse> removeFromWishlist(
      int productId) async {
    WishListRemoveResponse wishListRemoveResponse;
    String url =
        '${APIs.baseURL}${APIs.removeFromWishlist}?product_id=$productId';
  ...
  static Future<AllWishlistProducts> getWishlist(String lang) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        List<GetWishlist> wishlistLists =
            (result as List).map((data) => GetWishlist.fromJson(data)).toList();
